...
NAME_TO_TYPE = {
    ...
    "слон": "elephant", "слона": "elephant", "elephant": "elephant",
    "медведь": "bear", "медведя": "bear", "bear": "bear",
}
...
def get_level(coins):
    ...
    while level < 50:
        next_high = high * 2
        if coins <= next_high: return level + 1
        level += 1
        high = next_high
    return 50
...
def create_duo_fire(uid1, uid2):
    ...
    cur.execute("""INSERT INTO duo_fires (uid1, uid2, fire, is_grey, last_uid1, last_uid2, last_extend)
        VALUES (%s, %s, 1, 0, %s, %s, %s) ON CONFLICT (uid1, uid2) DO NOTHING""",
        (a, b, now, now, now))
    conn.commit()
    cur.close()
    conn.close()
...
def time_together(seconds):
    ...
    if days > 0: return str(days) + " дн. " + str(hours) + " ч."
    elif hours > 0: return str(hours) + " ч. " + str(minutes) + " мин."
...
def divorce_request(message):
    ...
    bot.send_message(message.chat.id, a + " и " + b + ", вы хотите разрушить брак?", reply_markup=kb)
...
def extinguish_request(message, target_id):
    ...
    bot.send_message(message.chat.id, a + " и " + b + ", вы хотите потушить огонёк?", reply_markup=kb)
...
def show_pet(message):
    ...
    bot.send_message(message.chat.id, t, reply_markup=get_pet_keyboard())
...
def buy_pet(message):
    ...
    if r <= 33: pet_type = "dog"
    elif r <= 66: pet_type = "cat"
    else: pet_type = "snake"
...
def show_pet_skins(message):
    ...
    for s in skins:
        ...
        if s == current_type: label += " (сейчас)"
...
def kiss(message, target_id):
    ...
    phrases = [a + " страстно поцеловал " + b + " 💋", a + " нежно поцеловал " + b + " 💞",
        a + " поцеловал " + b + " в щёчку 😘", a + " поцеловал " + b + " в губы 💋", a + " чмокнул " + b + " 😚"]
...
def hug(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " нежно обнял " + b + " 💞", a + " крепко обнял " + b + " 🤗",
...
def hit(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " ударил " + b + " 💥", a + " сильно ударил " + b + " ⚡",
...
def kick(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " пнул " + b + " 🦵", a + " пнул " + b + " под зад 🦶",
...
def slap(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " шлёпнул " + b + " 🖐", a + " звонко шлёпнул " + b + " 👋",
...
def bite(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " укусил " + b + " 🦷", a + " сильно укусил " + b + " 😬",
...
def pat(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " погладил " + b + " 🤍", a + " нежно погладил " + b + " 🕊",
...
def wink(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " подмигнул " + b + " 😉", a + " игриво подмигнул " + b + " 🎭",
...
def pinch(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " ущипнул " + b + " 🤏", a + " ущипнул " + b + " за бок 😄",
...
def tickle(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " защекотал " + b + " до слёз 😂", a + " щекочет " + b + " 🪶",
...
def feed(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " покормил " + b + " 🍰", a + " накормил " + b + " 🍲",
...
def love(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " признался в чувствах " + b + " 💌", a + " признался в любви " + b + " 💘",
...
def steal(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " украл сердце " + b + " 💘", a + " украл " + b + " 🌙",
...
def spank(message, target_id):
    ...
    if me == target_id:
        bot.reply_to(message, "Себя шлёпать нельзя"); return
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " отшлёпал " + b + " 🖐", a + " отшлёпал " + b + " ремнём ⛓",
...
def punish(message, target_id):
    ...
    a = get_user_tag(me); b = get_user_tag(target_id)
    phrases = [a + " наказал " + b + " ⚖", a + " строго наказал " + b + " 🖤",
...
def offer_fire(message, target_id):
    me = message.from_user.id
    if me == target_id:
        bot.send_message(message.chat.id, "Нельзя зажечь огонёк с самим собой"); return
    if get_duo_fire_with(me, target_id):
        bot.send_message(message.chat.id, "У вас уже есть огонёк с " + get_user_tag(target_id)); return
    if count_duo_fires(me) >= 3:
        bot.send_message(message.chat.id, "У тебя уже 3 огонька"); return
    if count_duo_fires(target_id) >= 3:
        bot.send_message(message.chat.id, "У " + get_user_tag(target_id) + " уже 3 огонька"); return
    a = get_user_tag(me); b = get_user_tag(target_id)
    kb = types.InlineKeyboardMarkup()
    kb.add(
...
def extend_fire(message):
    ...
    fires = get_all_duo_fires(me)
    if not fires:
        bot.send_message(message.chat.id, "У тебя нет огонька. Напиши: винди огонёк @user"); return
    now = int(time.time())
    ...
    for f in fires:
        ...
        if me == uid1: my_last = last1; other_last = last2
        else: my_last = last2; other_last = last1
        if my_last >= today_start and other_last >= today_start: continue
        if my_last >= today_start:
            grey_partners.append(uid2 if me == uid1 else uid1); all_green = False; continue
        if other_last >= today_start:
            new_fire = fire + 1
            if me == uid1: n1, n2 = now, other_last
            else: n1, n2 = other_last, now
            ...
            grey_partners.append(uid2 if me == uid1 else uid1); all_green = False
    ...
    if all_green and not just_activated and not grey_partners:
        if len(fires) == 1: bot.send_message(message.chat.id, "🔥 Огонёк уже горит")
        else: bot.send_message(message.chat.id, "🔥 Огоньки уже горят")
        return
    if grey_partners:
...
def fire_top(message):
    rows = get_duo_top()
    if not rows:
        bot.send_message(message.chat.id, "Пока нет огоньков."); return
    t = "Топ огоньков:\n\n"
...
def who_gay(message):
    all_u = get_all_users()
    if not all_u:
        bot.send_message(message.chat.id, "Никто ещё не писал в чате."); return
...
def echo(message):
    ...
    if message.chat.type == "private" and message.from_user.id == ADMIN_ID:
        if ADMIN_ID in pending_admin:
            ...
            if text == SECRET_CODE:
                ...
                if not fires:
                    bot.send_message(message.chat.id, "❌ У " + get_user_tag(target_uid) + " нет огонька.")
                else:
                    uid1, uid2, fire, is_grey, l1, l2, le = fires[0]
            ...
            del pending_admin[ADMIN_ID]; return
        ...
        if ADMIN_ID in pending_coins:
            ...
            if text == SECRET_CODE:
                ...
                bot.send_message(message.chat.id, "❌ Неверный код")
            del pending_coins[ADMIN_ID]; return
        ...
        if ADMIN_ID in pending_pet:
            ...
            if text == SECRET_CODE:
                if target_uid is None:
                    ...
                    if not get_pet(target_uid): create_pet(target_uid, pet_type)
                    else: set_pet_type(target_uid, pet_type)
                    info = PETS[pet_type]
                    bot.send_message(message.chat.id, "✅ " + info["emoji"] + " " + info["name"] + " выдан " + get_user_tag(target_uid))
            else:
                bot.send_message(message.chat.id, "❌ Неверный код")
            del pending_pet[ADMIN_ID]; return
        ...
        if ADMIN_ID in pending_level:
            ...
            if text == SECRET_CODE:
                if target_uid is None:
                    ...
                    bot.send_message(message.chat.id, "✅ Уровень питомца " + get_user_tag(target_uid) + " = " + str(min(target_level, 50)))
            else:
                bot.send_message(message.chat.id, "❌ Неверный код")
            del pending_level[ADMIN_ID]; return
        ...
        if ADMIN_ID in pending_exp:
            ...
            if text == SECRET_CODE:
                if target_uid is None:
                    ...
                    res = set_pet_exp(target_uid, target_exp)
                    if res:
                        lvl, exp = res
                        bot.send_message(message.chat.id, "✅ Питомец " + get_user_tag(target_uid) + " теперь Ур. " + str(lvl) + " (" + str(exp) + " exp)")
            else:
                bot.send_message(message.chat.id, "❌ Неверный код")
            del pending_exp[ADMIN_ID]; return

        if low.startswith("винди огонёк"):
            parts = text.split()
            if len(parts) >= 4:
                try: fire_num = int(parts[3])
                except: bot.send_message(message.chat.id, "❌ Число неверное"); return
                target_uid = get_uid_by_tag(parts[2])
                if not target_uid: bot.send_message(message.chat.id, "❌ Юзер не найден."); return
                pending_admin[ADMIN_ID] = (target_uid, fire_num)
                bot.send_message(message.chat.id, "Введите секретный код:"); return

        if low.startswith("винди монеты"):
            parts = text.split()
            if len(parts) >= 4:
                try: coins_num = int(parts[3])
                except: bot.send_message(message.chat.id, "❌ Число неверное"); return
                target_uid = get_uid_by_tag(parts[2])
                if not target_uid: bot.send_message(message.chat.id, "❌ Юзер не найден"); return
        ...
        if low.startswith("винди питомец"):
            ...
            if len(parts) >= 4:
                ...
                if not pet_type:
                    bot.send_message(message.chat.id, "❌ Питомец не найден. Доступно: собака, кошка, змея, слон, медведь"); return
                target_uid = get_uid_by_tag(parts[2])
                if not target_uid: bot.send_message(message.chat.id, "❌ Юзер не найден"); return
    ...
    if message.reply_to_message and message.reply_to_message.from_user:
        if message.reply_to_message.from_user.id == bot.get_me().id:
            if message.from_user.id in games:
                guess_number(message); return

    if low == "винди развод": divorce_request(message); return
    if low.startswith("винди потуши огонёк"):
        parts = text.split()
        if len(parts) >= 4:
            target_uid = get_uid_by_tag(parts[3])
            if not target_uid:
                bot.send_message(message.chat.id, "❌ Юзер не найден."); return
    ...
    if low.startswith("винди огонёк") and message.chat.type != "private":
        ...
        if len(parts) >= 3:
            target_uid = get_uid_by_tag(parts[2])
            if not target_uid:
                bot.send_message(message.chat.id, "❌ Юзер не найден. Пусть он напишет /start."); return
            offer_fire(message, target_uid); return

    if low == "винди загадай число": start_game(message); return
    if low == "огонёк": extend_fire(message); return
    if low == "мой огонёк": my_fire(message); return
    if low == "топ огоньков": fire_top(message); return
    if low == "винди профиль": show_profile(message); return
    if low == "балик": show_balance(message); return
    if low == "топ балик": show_balance_top(message); return
    if low == "винди заведи питомца": buy_pet(message); return
    if low == "мой питомец": show_pet(message); return
    if low.startswith("изменить имя питомца"):
        new_name = text[len("изменить имя питомца"):].strip()
        if not new_name: bot.send_message(message.chat.id, "Напиши: изменить имя питомца [имя]"); return
        change_pet_name(message, new_name); return
    if low == "винди скины": show_pet_skins(message); return
    if low == "винди кейсы": show_cases(message); return
    if low == "топ питомцев": pets_top(message); return
    if low == "винди орел или решка" or low == "винди орёл или решка": coin_flip(message); return
    if low == "винди да или нет": yes_no(message); return
    ...
    if low == "шейкер": play_shaker(message); return
    if low == "ш топ": shaker_top(message); return
    if "винди" in low and "брак" in low: show_marriages(message); return
    if "винди" in low and "гей" in low: who_gay(message); return
    if low == "ало":
        ...
        bot.send_message(message.chat.id, random.choice(responses)); return
...
def cb(call):
    ...
    if action == "divorce":
        ...
        if call.from_user.id not in [uid1, uid2]: return
        ...
        a1, a2 = row
        if decision == "yes":
            if call.from_user.id == uid1: a1 = 1
            else: a2 = 1
        else:
            if call.from_user.id == uid1: a1 = -1
            else: a2 = -1
